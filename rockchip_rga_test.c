...
static int rga_nv12_to_nv12_test(struct rga_test *test)
{
	...
	/* test */
	int test_fd;
	struct rockchip_bo *test_bo;
	test_bo = rockchip_create_buffer(dev, 172 * 231 * 4, 0);
	if (!test_bo) {
		fprintf(stderr, "Failed to create source fb!\n");
		return -EFAULT;
	}

	drmPrimeHandleToFD(dev->fd, test_bo->handle, 0 , &test_fd);	
	struct rga_image solid_img;
	solid_img.bo[0] = test_fd;
	solid_img.width = 172;
	solid_img.height = 231;
	solid_img.stride = 172 * 4;
	solid_img.buf_type = RGA_IMGBUF_GEM;
	solid_img.color_mode = DRM_FORMAT_XRGB8888;
	solid_img.fill_color = 0xaaaaaa;
	rga_solid_fill(ctx, &solid_img, 0, 0, 172 - 0 , 1 - 0);
	rga_solid_fill(ctx, &solid_img, 0, 1, 1- 0, 230 - 1);
	rga_solid_fill(ctx, &solid_img, 171, 1, 172 - 171 , 230 - 1);
    	rga_solid_fill(ctx, &solid_img, 0, 230, 172 - 0, 231 -230);
	rga_exec(ctx);
	getchar();
